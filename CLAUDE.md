...
- **QA gate in validation:** for code subtasks with `chain_to: qa`, the orchestrator's validation phase will not fire until the QA task has completed (helper `_find_qa_for_coder_subtask` searches all pipeline folders). If QA is still in-flight (`agents/qa/inbox/` or `processing/`), the parent is skipped for that cycle. When QA has finished, its verdict and result are injected into the subtask context passed to the LLM. Hard rule: if QA returned FAIL on its **first attempt** (`retry_count == 0`), the orchestrator immediately issues `redo` without calling the LLM — the LLM may only override a QA FAIL on retry attempts (`retry_count > 0`).
