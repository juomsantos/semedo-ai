...
- **Orphan recovery:** three functions run at orchestrator startup in sequence: `recover_orphaned_tasks()` scans `processing/` for tasks with `status: pending` (tasks whose decomposition never finished, e.g. killed mid-LLM-call) and moves them back to `inbox/` to be re-dispatched — `dispatched` parents are left alone. `recover_stalled_subtasks()` scans worker inboxes and `processing/` for subtasks whose parent is no longer active: if the parent is in `outbox/` with `status: complete`, the subtask is moved to `outbox/` (not `failed/`); if the parent is missing from both locations, the subtask is moved to `failed/`. `recover_orphaned_validation_subtasks()` sweeps `validation/` for stranded subtasks that the belt-and-suspenders glob missed (e.g. QA tasks and retry coders created before `parent_task_id` propagation was added, or any subtask whose parent completed just before a scheduler restart): if the subtask has a `parent_task_id` pointing to a `status: complete` parent in `outbox/`, it is moved to `outbox/`; if it has no `parent_task_id` but its `output_path` result file already exists in `outbox/`, it is also moved to `outbox/`.
